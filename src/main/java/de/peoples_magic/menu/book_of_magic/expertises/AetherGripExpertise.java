...
public class AetherGripExpertise extends BOMExpertise {


    public AetherGripExpertise(SpellArea spell_area, Player player, SpellTile tile, int z_index) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick, int adj_x, int adj_y, float zoom) {
        ...
        draw_expertise(guiGraphics, this.knowledge == 2, first_x, root_y, zoom, ABDUCTION_TEXTURE,
                mouseX, mouseY, List.of(Component.literal("Abduction")));
        draw_expertise(guiGraphics, this.knowledge == 3, second_x, root_y, zoom, HOMING_MISSLE_TEXTURE,
                mouseX, mouseY, List.of(Component.literal("Homing Missile")));
...
//                mouseX, mouseY, List.of(Component.literal("Dash")));
    ...
    public AttachmentType<Integer> get_attachment() {
        return ModAttachments.AETHER_GRIP_KNOWLEDGE.get();
    }
